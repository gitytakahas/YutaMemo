  ...
  //   source->ls();
   ...
   while ((key = (TKey*)nextkey())) {
	...
	std::cout << "key : "  << key->GetName() << std::endl;
	 ...
	 if(hh->GetEntries()==0){
	   std::cout << "0 entries ! : " <<  htype << " " << stype << " " << hh->GetSumOfWeights() << std::endl;
	   Int_t nbin = hh->GetXaxis()->GetNbins()/2.;
	   hh->SetBinContent(nbin+1, 0.00001);
	 }
	 
	 if(htype=="_W_" && stype=="muTau_vbf_tight"){ 
...
void copyFiles(TString filename){
  
  TString fname = filename;
  fname += ".new.root";
  TFile *f = new TFile(fname,"recreate");

  TString backupname = filename;
  backupname += ".root";
  CopyFile(backupname);
  //   f->ls();
  delete f;
