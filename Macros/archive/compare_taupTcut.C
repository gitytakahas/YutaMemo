...
void plot(TH1F *hist_signal[3], TH1F *hist[3], TH1F* hist_tau_signal[3], TH1F *hist_tau[3], TString title, TString period){
  ...
  for(int ican=0; ican < 3; ican++){
    ...
    leg[ican]->AddEntry(hist_tau[ican], catname, "");
    leg[ican]->AddEntry(hist_tau[ican], "Z #rightarrow #tau#tau", "lp");
    leg[ican]->AddEntry(hist_tau_signal[ican], "Signal", "lp");
    leg[ican]->AddEntry(hist_tau_signal[ican], legname, "");
    leg[ican]->Draw();
  }
